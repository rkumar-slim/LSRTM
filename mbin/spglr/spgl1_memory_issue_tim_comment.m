...
%    x = zeros(n,1);  %%% This line is not there in the Slim Version - Rajiv
else
   n     = length(x);
   realx = isreal(x) && isreal(b);
end
...
if isempty(x)   % can we delete g2  ???????? - Rajiv
    r         = b;  % r = b - Ax
...
lastFv(1) = f;
fBest     = f;
% xBest     = x; % commented out in SLIM-version - Rajiv
fOld      = f;
...
while 1
    ...
    % this is not in SLIm version upto line 494 - Rajiv
...
%        nnzIter = nnzIter + 1; % this is not in SLIM version - Rajiv
 %       if nnzIter >= activeSetIt, stat=EXIT_ACTIVE_SET; end % this is not
 %       in SLIM version - Rajiv
    ...
    %      if test2, stat=EXIT_BPSOL2_FOUND; end % Gradient zero -> BP sol.
       ...
       if testUpdateTau
           ...
           % Line 540-551 is not there in SLIM-SPGL1 - Rajiv
          ...
          if tau < tauOld
             ...
             % SLIM version dont have line from 572-574 - Rajiv
...
%              [f g g2] = funCompositeR(r, funForward, funPenalty, params);
    ...
    try
       ...
       nProdA = nProdA + localProdA; % this line is not in SLIM-SPGl1 version and also spgLinecurvey looks different
       ...
       % line from 680 to 738 is nt present in the SLIm version - Rajiv
...
%       gOld = g;
       ...
       y    = g - gOld;
       sts  = dot(xOld,xOld); % instead of dot computing norm(xOld)^2 in SLIM version, i dont know whether norm computing is faster then dot?? - Rajiv
       sty  = dot(xOld,y);
    ...
    if singleTau || f > sigma % Don't update if superoptimal.
       ...
       if fBest > f
          fBest = f;
%           xBest = x; % commented out from SLIM-SPGL1 version - Rajiv
       end
    end
